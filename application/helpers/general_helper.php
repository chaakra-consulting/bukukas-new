...
if (!function_exists('encode_id')) {
    ...
    function encode_id($id, $salt) {
        ...
        $data = openssl_encrypt($id, 'aes-256-cbc', $key, OPENSSL_RAW_DATA);
        $base64 = base64_encode($data);
        $modified = str_replace(['=', '+', '/'], ['~', '_', '-'], $base64);
        return $modified;
      }
...
if (!function_exists('decode_id')) {
    ...
    function decode_id($id, $salt) {
        ...
        $key = hash('sha256', $salt);
        $decrypted = openssl_decrypt($decoded, 'aes-256-cbc', $key, OPENSSL_RAW_DATA);
        if ($decrypted && strpos($decrypted, $salt) !== false) {
...
if (!function_exists('get_s_payment_making_data')) {
    ...
    function get_s_payment_making_data($invoice_id) {
        ...
        if ($invoice_info) {
            ...
            $data['client_info'] = $ci->Master_Customers_model->get_one($data['invoice_info']->fid_cust);
            $data['mark_info'] = $ci->Master_Marketing_model->get_one($data['invoice_info']->marketing);
...
if (!function_exists('get_p_invoices_making_data2')) {
    ...
    function get_p_invoices_making_data2($invoice_id) {
        ...
        if ($invoice_info) {
            $data['invoice_info'] = $invoice_info;
            $data["order_info"] = $ci->Purchase_Request_model->get_details(array("id" => $data['invoice_info']->fid_order ))->row();
...
if (!function_exists('get_offer_making_data')) {

    function get_offer_making_data($offer_id) {
        $ci = get_instance();
        $quot_id = $ci->Sales_Offer_model->get_details(array("id" => $offer_id))->row();
        if ($quot_id) {
            $data['invoice_info'] = $quot_id;
            $data['mark_info'] = $ci->Master_Marketing_model->get_one($data['invoice_info']->marketing);
            $data['pers_info'] = $ci->Master_Perusahaan_model->get_one($data['invoice_info']->fid_custt);
            $data['client_info'] = $ci->Master_Customers_model->get_one($data['invoice_info']->fid_cust);
            $data['invoice_items'] = $ci->Sales_OfferItems_model->get_details(array("fid_offer" => $offer_id))->result();
            $data['invoice_status_label'] = get_order_status_label($offer_id);
            $data["invoice_total_summary"] = $ci->Sales_Offer_model->get_offer_total_summary($offer_id);
...
if (!function_exists('get_order_making_data')) {
    ...
    function get_order_making_data($order_id) {
        ...
        if ($quot_id) {
            ...
            $data['client_info'] = $ci->Master_Customers_model->get_one($data['invoice_info']->fid_cust);
            $data['pers_info'] = $ci->Master_Perusahaan_model->get_one($data['invoice_info']->fid_custt);
            $data['invoice_items'] = $ci->Sales_OrderItems_model->get_details(array("fid_orderr" => $order_id))->result();
...
if (!function_exists('prepare_s_invoice_pdf_jalan')) {
    ...
    function prepare_s_invoice_pdf_jalan($invoice_data, $mode = "download_jalan") {
        ...
        if ($invoice_data) {
            ...
            $html = $ci->load->view("invoice/inv_pdf_jalan", $invoice_data, true);
...
if (!function_exists('prepare_offer_pdf')) {

    function prepare_offer_pdf($invoice_data, $mode = "download") {
        $ci = get_instance();
        $ci->load->library('pdf');
        $ci->pdf->setPrintHeader(false);
        $ci->pdf->setPrintFooter(false);
        ...
        if ($invoice_data) {
            ...
            $html = $ci->load->view("offer/offer_pdf", $invoice_data, true);
            ...
            $invoice_info = get_array_value($invoice_data, "invoice_info");
            $pdf_file_name = "PENAWARAN_" .stripslashes($invoice_info->code). ".pdf";
...
if(!function_exists('getIdNo')){

    function getIdNo($table,$prefix = ''){
        $ci = get_instance();
...
function getPersInfo($id){
    $ci = get_instance();
    $ci->db->where("id",$id);
    $data = $ci->db->get('master_perusahaan')->row();
...
function getKodeInfo($id){
    $ci = get_instance();
    $ci->db->where("id",$id);
    $data = $ci->db->get('kode_kas')->row();
    return $data->name." - " .$data->company_name;
}
