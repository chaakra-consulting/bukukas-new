...
class Signin extends CI_Controller {
    ...
    public function sso_login()
    {
        ...
        if (isset($_POST['user_app_id'])) {
            ...
            $getAdmin = $this->db->where('id', $user_app_id)->get('users')->row_array();

            $this->session->set_userdata([
                'masuk'   => TRUE,
                'user_id' => $getAdmin['id'],
                'user_name' => $getAdmin['first_name'] . ' ' . $getAdmin['last_name'],
                'user_type'  => $getAdmin['user_type'],
                'is_admin' => $getAdmin['is_admin'],
            ]);
        ...
        if ($getRespon->success == true) {
            if ($getRespon->data_app != null) {
                $getAdmin = $this->db->where('id', $getRespon->data_app->user_app_id)->get('users')->row_array();
                ...
                $getRespon->redirect = base_url('dashboard');
            }
        }
        sleep(1);
